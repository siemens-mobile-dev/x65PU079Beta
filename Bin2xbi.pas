...
var
    ...
    hSWPFile : THandle = INVALID_HANDLE_VALUE; // SWPEXE базового файла ('SWPEXE414.SRC')
    outfilename : string;
    xbzfilename : string;
    binfilename : string;  // User Bin файла прошивки
    swpfilename : string;  // '.\SWPEXE414.SRC'
//    wsefilename : string;  //
//    sBinImei : string = '??????????????';
    HWbinFile : integer =0;
    TabSegBin : array[0..512] of byte;
    TSw : array[0..1] of tTabSwup = (
     (name : 'SWPR65V414'; // V4.14
      fname : 'SWPR65V414.SRC';
      size : 372736;
      key : ($27440,$290DC,$41D58,0);
      str  : $35A96),
     (name : 'SWPX75V415'; // V4.15
      fname : 'SWPX75V415.SRC';
      size : 393216;
      key : ($28438,$2A0DC,$472C0,0);
      str  : $3A9EE)
{     (name : 'SWPX75'; // V4.12
      fname : 'SWPX75V412.SRC';
      size : 372736;
      key : ($27440,$290DC,$3C0B0,0);
      str  : $2FF26) }
    );
...
{
function TFormBin2Swp.OpenSwpSRC : boolean;
begin
    result:=False;
...
begin
    result:=False;
    swn:=0; // R65
    if NewSGold then swn:=1; // X75
//    SetLength(sbuf):=39;
    if hSwpFile <> INVALID_HANDLE_VALUE then begin
     FileClose(hSwpFile);
     hSwpFile := INVALID_HANDLE_VALUE;
    end;
    SwpFileName := IniFile.ReadString('Setup',TSw[swn].name,'.\'+TSw[swn].fname);
    hSwpFile := FileOpen(SwpFileName,fmOpenRead or fmShareCompat);
    if hSwpFile = INVALID_HANDLE_VALUE then begin
     with FormMain.OpenDialog do begin
      FilterIndex:=1;
      FileName := IniFile.ReadString('Setup',TSw[swn].name,'.\'+TSw[swn].fname);
      ...
      Title:='Укажите исходный файл "'+TSw[swn].fname+'".';
     end;//with OpenDialog
     if FormMain.OpenDialog.execute then begin
       SwpFileName := FormMain.OpenDialog.FileName;
       hSwpFile := FileOpen(SwpFileName,fmOpenRead or fmShareCompat);
       if hSwpFile = INVALID_HANDLE_VALUE then begin
        sSwp_Err:='Нет исходного файла "'+SwpFileName+'"!';
        exit;
       end;
     end
     else begin
    ...
    sizeswpfile:= FileSeek(hSwpFile,0,2);
    if (sizeswpfile<300000) or (sizeswpfile>500000) then begin
     FileClose(hSwpFile);
     hSwpFile := INVALID_HANDLE_VALUE;
...
//    SWPMemoryStream.SetSize(i);
    ...
    SetLength(sbuf,16);
    i:=0;
    while(TSw[swn].key[i]<>0) do begin
     ; //$27460
     if (SWPMemoryStream.Seek(TSw[swn].key[i], soFromBeginning)<>TSw[swn].key[i])
     or (SWPMemoryStream.Read(sbuf[1],16) <> 16) then begin
     ...
     if (SWPMemoryStream.Seek(TSw[swn].key[i]+$18, soFromBeginning)<>TSw[swn].key[i]+$18)
    ...
    SWPMemoryStream.Seek(TSw[swn].str,soFromBeginning);
    sbuf:='12';
    ...
    sbuf:=sbuf+') V%d.%02d';
    SWPMemoryStream.Seek(TSw[swn].str-1,soFromBeginning);
    ...
    SWPMemoryStream.Seek(0,soFromBeginning);
////
    IniFile.WriteString('Setup',TSw[swn].name,SwpFileName);
    result:=True;
{////
}
end;
...
//   if NOT OpenSwpFile(s+' '+EditDataBaseName.text) then begin
   ...
   if NOT OpenSwpFile(sBaseName,FormMain.BootKey) then begin
    ...
    FormMain.AddLinesLog('http://papuas.allsiemens.com/swp3.rar (~260 KB)');
