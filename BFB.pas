...
begin
   ...
   if BFB_Error=BFB_OK then begin
    BFB_Error:=ReadBFB;
    if BFB_Error=BFB_OK then begin
     if ((ibfb.head.len=0) or (ibfb.code.cmdb<>obfb.code.cmdb)) then BFB_Error:=ERR_BFB_RD_CMD;
    end;
   end;
   result:=BFB_Error;
end;
   ...
   if BFB_Error=BFB_OK then begin
    ...
    if BFB_Error=BFB_OK then begin
     if ((ibfb.head.len=0) or (ibfb.code.cmdb<>obfb.code.cmdb)) then BFB_Error:=ERR_BFB_RD_CMD
      ...
      if ibfb.data[ibfb.head.len-1]<>chk then BFB_Error:=ERR_BFB_CHK;
     end;
    end;
   end;
   result:=BFB_Error;
...
begin
  ...
  BFB_Error:=ReadBFB;
  if BFB_Error<>BFB_OK then exit;
  if not ((ibfb.head.len<>0) and (ibfb.code.cmdb=$A3)) then exit;
  Buf[0]:=$C0;
...
begin
  ...
  if (ibfb.head.len=$2)and(ibfb.data[1]=$2) then BFB_Error:=ERR_BFB_EEPBLKNONE
